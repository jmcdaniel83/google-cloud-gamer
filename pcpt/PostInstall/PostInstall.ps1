...
$currentusersid = Get-LocalUser "$env:USERNAME" | Select-Object SID | Format-Table -HideTableHeaders | Out-String | ForEach-Object { $_.Trim() }
...
function force-close-apps {
    ...
    if ( ( $null -ne (Get-Item -Path "HKCU:\Control Panel\Desktop").GetValue("AutoEndTasks") ) -eq $true ) {
...
Function Server2019Controller {
    ...
    if ((gwmi win32_operatingsystem | % caption) -like '*Windows Server 2019*') {
        ...
        cmd.exe /c '"C:\Program Files\Parsec Virtual Display Driver\devcon.exe" dp_add "C:\ParsecTemp\Drivers\Xbox360_64Eng\xusb21.inf"' | Out-Null
        }
    }
...
function disable-devices {
    ...
    Start-Process -FilePath "C:\Program Files\Parsec Virtual Display Driver\devcon.exe" -ArgumentList '/r disable "HDAUDIO\FUNC_01&VEN_10DE&DEV_0083&SUBSYS_10DE11A3*"'
    ...
    Start-Process -FilePath "C:\Program Files\Parsec Virtual Display Driver\devcon.exe" -ArgumentList '/r disable "PCI\VEN_1013&DEV_00B8*"'
    Start-Process -FilePath "C:\Program Files\Parsec Virtual Display Driver\devcon.exe" -ArgumentList '/r disable "PCI\VEN_1D0F&DEV_1111*"'
    Start-Process -FilePath "C:\Program Files\Parsec Virtual Display Driver\devcon.exe" -ArgumentList '/r disable "PCI\VEN_1AE0&DEV_A002*"'
    }
